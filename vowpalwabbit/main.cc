...
{ try
  {
    ...
    if (argc == 3 && !strcmp(argv[1], "--args"))
    {
      ...
      { std::stringstream sstr;
        ...
        alls.push_back(&setup(l_argc, l_argv)); //parsing regressor and vector::push_back // Adds a new element at the end of the vector
      }
    }
    else
    {
    	alls.push_back(&setup(argc, argv)); // adding &vw objects to the end of alls vector<vw*> after setting it up with argc and argv
    }
